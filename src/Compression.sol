...
library Compression {
    ...
    function compressZeros(bytes memory input) internal pure returns (bytes memory ret) {
        ...
        for (uint256 i; i <= inputLength; ++i) {
            ...
            if (_byte == 0) {
                ...
                if ((inZeroSegment) && (i - zeroSegments[idx].start > TOLERANCE)) {
    ...
    // an unchecked function but the calling context in this contract is safe.. methinks...
